...
public enum FJ_AlignType {
    ...
    private func layoutAttributes(isInner: Bool, isVertical: Bool) -> FJ_LayoutAttributes {
        let attributes = FJ_LayoutAttributes()
        ...
        switch self {
            ...
            // 仅内部参照需要
            case .Center:
                return FJ_LayoutAttributes(horizontal: .CenterX, referHorizontal: .CenterX, vertical: .CenterY, referVertical: .CenterY)
        }
    }
}
...
extension UIView {
    ...
    :returns: 约束数组
    */
    public func fj_Fill(referView: UIView, insets: UIEdgeInsets = UIEdgeInsetsZero) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    public func fj_AlignInner(type type: FJ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint]  {
        
        return fj_AlignLayout(referView, attributes: type.layoutAttributes(true, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func fj_AlignVertical(type type: FJ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return fj_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: true), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func fj_AlignHorizontal(type type: FJ_AlignType, referView: UIView, size: CGSize?, offset: CGPoint = CGPointZero) -> [NSLayoutConstraint] {
        
        return fj_AlignLayout(referView, attributes: type.layoutAttributes(false, isVertical: false), size: size, offset: offset)
    }
    ...
    :returns: 约束数组
    */
    public func fj_HorizontalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        firstView.fj_AlignInner(type: FJ_AlignType.TopLeft, referView: self, size: nil, offset: CGPoint(x: insets.left, y: insets.top))
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.fj_sizeConstraints(firstView)
            subView.fj_AlignHorizontal(type: FJ_AlignType.TopRight, referView: preView, size: nil, offset: CGPoint(x: insets.right, y: 0))
            preView = subView
        }
    ...
    :returns: 约束数组
    */
    public func fj_VerticalTile(views: [UIView], insets: UIEdgeInsets) -> [NSLayoutConstraint] {
        ...
        for i in 1..<views.count {
            let subView = views[i]
            cons += subView.fj_sizeConstraints(firstView)
            subView.fj_AlignVertical(type: FJ_AlignType.BottomLeft, referView: preView, size: nil, offset: CGPoint(x: 0, y: insets.bottom))
            preView = subView
        }
    ...
    :returns: 对应的约束
    */
    public func fj_Constraint(constraintsList: [NSLayoutConstraint], attribute: NSLayoutAttribute) -> NSLayoutConstraint? {
    ...
    :returns: 约束数组
    */
    private func fj_AlignLayout(referView: UIView, attributes: FJ_LayoutAttributes, size: CGSize?, offset: CGPoint) -> [NSLayoutConstraint] {
        
        translatesAutoresizingMaskIntoConstraints = false
        
        var cons = [NSLayoutConstraint]()
        
        cons += fj_positionConstraints(referView, attributes: attributes, offset: offset)
        
        if size != nil {
            cons += fj_sizeConstraints(size!)
        }
    ...
    :returns: 约束数组
    */
    private func fj_sizeConstraints(size: CGSize) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func fj_sizeConstraints(referView: UIView) -> [NSLayoutConstraint] {
    ...
    :returns: 约束数组
    */
    private func fj_positionConstraints(referView: UIView, attributes: FJ_LayoutAttributes, offset: CGPoint) -> [NSLayoutConstraint] {
...
///  布局属性
private final class FJ_LayoutAttributes {
